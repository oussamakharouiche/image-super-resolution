...
def run_inference(model, dataloader, device, T=T, output_dir="test_output_attn_DDIM"):
    ...
    with torch.no_grad():
        for batch_idx, (hr_imgs, lr_imgs, paths) in enumerate(tqdm(test_loader)):
            ...
            for t in list(reversed(range(T))):
                ...
                prev_alpha_bar_t = cumulative_alphas[t-1] if t>=1 else torch.tensor(1.0)
                
                # Predict noise for the entire batch
                noise_p = model(x, torch.tensor([t]).repeat(batch_size).to(device))
                # noise_p = torch.nn.Tanh()(noise_p)

                print_norm("noise", noise_p)
                
                # Update predictions
                img_pred = (
                    torch.sqrt(prev_alpha_bar_t) * (
                        (img_pred - torch.sqrt(1-current_alpha_bar_t) * noise_p) / torch.sqrt(current_alpha_bar_t)
                    ) + 
                    torch.sqrt(1-prev_alpha_bar_t) * noise_p
                )
